...
/*  Copyright 2012  gCodeLabs  (email : admin@gcodelabs.com)
    ...
    Foundation, Inc., 51 Franklin St, Fifth Floor, Boston, MA  02110-1301  USA  a
*/
...
function fb_photos_options_page() {
	...
	echo '<table class="form-table" style="width:730px;">';
	echo '<tr valign="top">';
	echo '<th scope="row">' . __('Facebook Account Owner ID :', 'fbphotos') . '</th>';
	echo '<td><input type="text" name="fb_owner_id" value="' . get_option('fb_owner_id') . '" /> <b> (example : 100002245703208)</b> </td>';
	echo '</tr>';
	echo '<tr valign="top">';
	echo '<th scope="row">' . __('Page Link :', 'fbphotos') . '</th>';
	echo '<td><input type="text" name="fb_page_link" value="' . get_option('fb_page_link') . '" /><b>(example : http://www.domain.com/sample-page/)</b> where you want to show the facebook gallery.</td>';
	echo '</tr>';
	echo '</table>';
	echo '<p class="submit">';	
